...
# Upgraded to 1.5B model for significantly better reasoning and accuracy
...
class GenerationEngine:
    ...
    def format_prompt(self, query: str, context_chunks: List[str]) -> List[Dict[str, str]]:
        ...
        elif not isinstance(context_chunks, (list, tuple)):
            context_block = str(context_chunks)
        else:
            # Ensure all items are strings and filter None
            safe_chunks = [str(c) for c in context_chunks if c is not None]
            context_block = "\n---\n".join(safe_chunks)
