...
class IngestionEngine:
    ...
    def __init__(self, output_root: str = "rag_storage"):
        ...
        # Initialize Pipeline Options with ALL features enabled
    ...
    def process_pdf(self, pdf_path: str, page_range: Optional[Tuple[int, int]] = None) -> str:
        ...
        # Note: We use convert_all and iterate to potentially catch start/end events
        logger.info("Initializing Docling Pipeline (Loading models into memory)...")
        conv_res_iter = self.converter.convert_all(
            [pdf_path], 
            page_range=page_range if page_range else (1, sys.maxsize)
        )
        
        logger.info("Parsing document structure and running AI models (OCR/Layout/Formulas)...")
        result = next(conv_res_iter)
        
        num_pages = len(result.pages)
        logger.info(f"Successfully parsed {num_pages} pages.")

        # 2. Extract and Save Pictures
        logger.info("Extracting and optimizing images...")
        ...
        # 5. Perform Enriched Hierarchical Chunking
